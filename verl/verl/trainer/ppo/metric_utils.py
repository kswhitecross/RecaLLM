    ...
    for data_source, uid2var2vals in data_src2uid2var2vals.items():
        ...
        for uid, var2vals in uid2var2vals.items():
            ...
            for var_name, var_vals in var2vals.items():
                ...
                # skip all-NaN variables (non-applicable category metrics)
                try:
                    arr = np.asarray(var_vals, dtype=np.float64)
                    if np.all(np.isnan(arr)):
                        continue
                except (TypeError, ValueError):
                    pass

                # compute mean and std (use nanmean/nanstd to handle per-sample NaN)
                n_resps = len(var_vals)
                metric = {f"mean@{n_resps}": float(np.nanmean(var_vals))}

                if n_resps > 1:
                    metric[f"std@{n_resps}"] = float(np.nanstd(var_vals))
