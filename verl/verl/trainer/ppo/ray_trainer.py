...
class RayPPOTrainer:
    ...
    def _log_rollout_data(
        ...
        with marked_timer("dump_rollout_generations", timing_raw, color="green"):
            ...
            if "advantages" in batch.batch:
                ...
                # Per-completion mean advantage (masked over valid response tokens)
                mean_adv = (adv * mask).sum(-1) / mask.sum(-1).clamp(min=1)
                reward_extra_infos_to_dump["advantage"] = mean_adv.cpu().tolist()
            if "request_id" in batch.non_tensor_batch:
                reward_extra_infos_dict.setdefault(
                    "request_id",
                    batch.non_tensor_batch["request_id"].tolist(),
                )
            # Log data_source for per-dataset-type graphs in print_completions.py
            if "data_source" in batch.non_tensor_batch:
                reward_extra_infos_to_dump["data_source"] = list(batch.non_tensor_batch["data_source"])
            # Log response_length (valid response tokens) for completion length tracking
            prompt_length = batch.batch["prompts"].shape[-1]
            response_lengths = batch.batch["attention_mask"][:, prompt_length:].sum(-1).cpu().tolist()
    ...
    def fit(self):
        ...
        # Route file logger to the run's save directory so training metrics
        # are co-located with checkpoints and rollouts.
        if "file" in (self.config.trainer.logger or []) and not os.environ.get("VERL_FILE_LOGGER_PATH"):
            os.environ["VERL_FILE_LOGGER_PATH"] = os.path.join(
                self.config.trainer.default_local_dir, "training_metrics.jsonl"
            )
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # Log extra reward info (e.g., answer_score, has_answer, correct_format)
                if reward_extra_infos_dict:
                    for key, values in reward_extra_infos_dict.items():
                        if key != "score":  # score is already logged as critic/score/*
                            try:
                                # Convert to numpy array and compute mean (handles bool, int, float, np.bool_, etc.)
                                arr = np.asarray(values, dtype=np.float64)
                                metrics[f"reward_extra/{key}"] = float(np.mean(arr))
