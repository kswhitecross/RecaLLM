...
```bash
python -m recallm.datasets.create_datasets \
...
|-------------|---------------|-------|
| `msmarco_v2` | MSMARCO V2 passage corpus | `python -m recallm.datasets.reranking.prepare_data` |
| `qampari` | QAMPARI DPR retriever data | `python -m recallm.datasets.citation_qa.download_qampari` |
...
python -m recallm.datasets.qa_kilt.build_dense_embeddings \
    --qa_kilt_source_dir /path/to/kilt \
    --qa_kilt_window_dir /path/to/kilt_windows

# Build FAISS indexes
python -m recallm.datasets.qa_kilt.build_faiss_index \
    --qa_kilt_source_dir /path/to/kilt \
    --qa_kilt_window_dir /path/to/kilt_windows
```
