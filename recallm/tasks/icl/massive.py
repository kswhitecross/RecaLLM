"""
Raw Amazon MASSIVE dataset for ICL training.

Loads MASSIVE (en-US) from HuggingFace, selects test examples, and builds
...
class MassiveDataset(Dataset):
    """Raw Amazon MASSIVE dataset. Loads from HF, provides ICLExample instances."""
    ...
    def __init__(self, n_examples: int, seed: int = 0, max_demos: int = 2000):
        ...
        # Load English subset from HuggingFace (parquet revision for datasets>=4.0 compat)
        ds = load_dataset("AmazonScience/massive", split="train", revision="refs/convert/parquet")
        ds = ds.filter(lambda x: x["locale"] == "en-US")
        self.texts: list[str] = ds["utt"]
        self.labels: list[int] = ds["intent"]
        self.label_names: list[str] = ds.features["intent"].names
